...
/** Workspace arrangement: preset plus pane proportions. This is the domain type; the saved-view
  ...
  * width for the left/right inspectors and of the centre width for the Hybrid divider; all in (0,
  * 1).
  *
  * The product definition's lower drawer (design · contrast · diagnostics · clusters · provenance
  * graph) has no fraction here on purpose: a saved-view version is immutable and a share link never
  * changes meaning, so a number for a region nothing renders would be persisted into links forever
  * and could never be reinterpreted. The payload is an open record, so the fraction returns with
  * the drawer.
  */
final case class WorkspaceLayout(
  ...
  def isValid: Boolean =
    List(navigatorFraction, inspectorFraction, splitFraction).forall(f =>
      f > 0.0 && f < 1.0
    ) && navigatorFraction + inspectorFraction < 1.0

object WorkspaceLayout:
  val default: WorkspaceLayout = WorkspaceLayout(LayoutPreset.Volume, 0.18, 0.25, 0.5)
