...
object ByteProfile:
  ...
  /** `pointer` is the JSON Pointer of the enclosing value when the scanner knows it (duplicate
    * keys); `""` for byte-level violations of the whole document.
    */
  final case class Violation(offset: Int, message: String, pointer: String = ""):
    def render: String = s"byte $offset: $message"
    def problem: Problem = Problem(pointer, render)
  ...
  private[json] object Scanner:
    ...
    private var depth = 0
    private var path: List[String] = Nil // reversed pointer tokens of the value being scanned
    ...
    private def obj(
      ...
      while true do
        ...
        if !seen.add(key) then
          out += Violation(
            keyAt,
            s"duplicate object key \"$key\"",
            JsonPointer.of(path.reverse)
          )
        i = skipWs(s, after)
