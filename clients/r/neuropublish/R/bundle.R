...
#' `jsonlite::toJSON(auto_unbox = TRUE, digits = NA, null = "null", pretty = TRUE)`
#' under the protocol's byte profile: UTF-8 without a byte-order mark, one root
#' object, and finite numbers only. A `NaN`, `NA`, `Inf` or `-Inf` anywhere in
#' the manifest is refused with an error naming its JSON Pointer, because JSON
#' cannot represent it and the server would reject the bytes.
#'
...
np_stage_asset <- function(a, dir) {
  ...
  if (ext == ".") ext <- ""
  rel <- file.path("assets", paste0(np_safe_file_name(a$id), ext))
...
np_manifest_bytes <- function(manifest) {
  np_check_finite(manifest, "")
  text <- jsonlite::toJSON(manifest, auto_unbox = TRUE, digits = NA, null = "null", pretty = TRUE)
